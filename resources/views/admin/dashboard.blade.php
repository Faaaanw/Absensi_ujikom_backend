    ...
    <div class="col-lg-8 mb-4">
        ...
        <div class="card border-0 shadow-sm rounded-4">
            <div class="card-header bg-white border-bottom-0 pt-4 px-4">
                <h5 class="fw-bold text-secondary mb-0"><i class="fa-solid fa-list-check me-2"></i>Log Absensi Terakhir</h5>
            </div>
                                        ...
                                        <div class="d-flex align-items-center">
                                            <div class="avatar-sm me-2">
                                                <span class="avatar-title rounded-circle bg-light text-primary fw-bold p-2">
                                                    {{ substr($row->user->profile->full_name, 0, 1) }}
                                                </span>
                                            </div>
                                            <span class="fw-bold text-dark">{{ $row->user->profile->full_name }}</span>
                                        </div>
                                    </td>
                                    <td class="text-muted small">
                                        <i class="far fa-clock me-1"></i> {{ $row->clock_in }}
                                    </td>
                                    <td class="px-4 text-end">
                                        @if($row->status == 'hadir')
                                            <span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 rounded-pill px-3">HADIR</span>
                                        @else
                                            <span class="badge bg-warning bg-opacity-10 text-warning border border-warning border-opacity-25 rounded-pill px-3">TERLAMBAT</span>
                                        @endif
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3" class="text-center py-4 text-muted small">Belum ada data absensi hari ini.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <div class="col-lg-4">
        <div class="card border-0 shadow-sm rounded-4 text-center overflow-hidden">
            ...
            <div class="card-body bg-light p-4">
                <div class="scanner-wrapper position-relative bg-dark rounded-3 overflow-hidden shadow-inner" style="min-height: 250px;">
                    
                    <div id="camera-placeholder" class="d-flex flex-column align-items-center justify-content-center h-100 position-absolute w-100 top-0 start-0 text-white-50">
                ...
                <div class="mt-4">
                    <button id="btn-start" class="btn btn-primary w-100 rounded-pill fw-bold shadow-sm mb-2" onclick="startScanner()">
                        <i class="fa-solid fa-power-off me-2"></i> Nyalakan Kamera
                    </button>
                    <button id="btn-stop" class="btn btn-danger w-100 rounded-pill fw-bold shadow-sm d-none" onclick="stopScanner()">
            ...
            <div class="card-body p-3">
                <div class="d-flex">
                    ...
                    <div class="small text-muted">
                        Pastikan karyawan menunjukkan QR Code dengan jelas di depan kamera. Sistem akan otomatis mencatat jam kehadiran.
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function startScanner() {
        ...
        // Start scanning
        html5QrCode.start(
            { facingMode: "environment" }, // Pakai kamera belakang jika ada
            config,
            onScanSuccess,
            onScanFailure
        ).catch(err => {
            ...
            stopScanner(); // Reset UI jika gagal
        });
    }
    ...
    function onScanSuccess(decodedText, decodedResult) {
        // 1. Bunyikan Beep (Opsional, UX bagus)
        // let audio = new Audio('path/to/beep.mp3'); audio.play();

        // 2. Pause kamera sebentar biar gak double scan
        html5QrCode.pause(); 
        resultElement.innerText = "Memproses: " + decodedText;
        ...
        fetch('/api/attendance/submit-scan', {
            ...
            headers: {
                ...
                // 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content') // Jika perlu CSRF
            },
        ...
        .then(data => {
            if (data.success) {
                ...
                // Jika gagal, resume scanning biar bisa coba lagi tanpa reload
                html5QrCode.resume(); 
    ...
    function onScanFailure(error) {
        // Biarkan kosong agar tidak spam console log saat tidak ada QR
    }
</script>
@endsection